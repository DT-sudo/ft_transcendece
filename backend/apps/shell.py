...
def _nav_links(user, active: str) -> list[dict[str, Any]]:
    if not user.is_authenticated:
        return []
    items = [("manager_shifts", "Shifts"), ("manager_employees", "Team")] if user.is_manager else [("employee_shifts", "My Shifts")]
...
def render_app(request: HttpRequest, *, page: str, title: str, data: dict[str, Any] | None = None, nav_active: str = "") -> HttpResponse:
    bootstrap = {
        ...
        "urls": {"logout": reverse("logout"), "privacy": reverse("privacy_policy"), "terms": reverse("terms_of_service")},
